...
const LIMPIEZA_SEMANAL = [
  ...
  { title: 'Bajar el reciclaje (vidrio, papel, plásticos)' }
];
...
async function main() {
  ...
  for (const c of NEW_COMPRAS) {
    const exists = allTasks.some(t => {
      ...
      return (p.title||'').toLowerCase().includes(c.title.split(' ')[0].toLowerCase()) && (p.categoryId === 'compra' || p.category_id === 'compra');
    });
    if (!exists) {
      const payload = {
        id: crypto.randomUUID(),
